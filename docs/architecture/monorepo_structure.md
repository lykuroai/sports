...
> **【2026-06-26 更新・本ドキュメントは旧構成案（履歴）】**
> 本書のマルチアプリ＋サブドメイン構成（web/account/golf/running/outdoor/facility/admin）は
> **統合サイト化により廃止**。現在のアプリは **`apps/web`（統合サイト, spotomo.lykuro.ai）と
> `apps/admin`（管理画面, admin-spotomo.lykuro.ai）の2つのみ**で、種目・ログイン・施設運営者・課金は
> すべて web 内のパスに集約済み。最新の構成・方針は **`docs/仕様変更/0_移行設計_統合サイト化.md`** と
> **`CLAUDE.md`「統合サイト化」節**を参照。以下は当初の段階的モノレポ案の記録。
