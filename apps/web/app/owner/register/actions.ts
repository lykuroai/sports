...
export async function registerOwner(_prev: AuthState, formData: FormData): Promise<AuthState> {
  ...
  // 単一オリジン: 確認メールのコールバックは自オリジン。next=/owner で確認後は運営者ログインへ。
  ...
  const { data, error } = await supabase.auth.signUp({
    ...
    options: {
      ...
      emailRedirectTo: `${origin}/auth/callback?verify=email&next=${encodeURIComponent("/owner")}`,
    },
  });
  if (error) return { error: error.message };

  if (!data.session) redirect("/owner/login?notice=check-email");
  redirect("/owner");
}
