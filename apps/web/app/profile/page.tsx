...
// 決済情報・本人確認・通知・退会の各画面は account アプリに残るため、絶対URLで誘導する
// （サブドメイン間で Cookie 共有。COOKIE_DOMAIN=.lykuro.ai）。
const ACCOUNT_URL = process.env.NEXT_PUBLIC_ACCOUNT_URL ?? "";
const acct = (path: string) => `${ACCOUNT_URL}${path}`;
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6">
      ...
      <nav className="card divide-y p-2 text-sm">
        <a className="block px-3 py-2 hover:bg-slate-50" href={acct("/notifications")}>通知一覧</a>
        <a className="block px-3 py-2 hover:bg-slate-50" href={acct("/settings/notifications")}>通知設定</a>
        <a className="block px-3 py-2 hover:bg-slate-50" href={acct("/billing")}>決済情報</a>
        <a className="block px-3 py-2 hover:bg-slate-50" href={acct("/verification")}>本人確認</a>
        <a className="block px-3 py-2 text-red-600 hover:bg-slate-50" href={acct("/withdraw")}>退会</a>
      </nav>
    </div>
  );
}
