...
import { isFavorited, isFollowing, fetchActivityEligibility } from "@spotomo/domain-common";
import { VerifyNotice } from "../verify-notice";
import { fetchEventDetail, isApplyable } from "../../../lib/events";
import { applyToEvent, messageOrganizer } from "../actions";
...
export default async function EventDetail({
  ...
  const loginHref = user ? "" : await loginUrlFor(`/recruitments/${ev.id}`);
    ...
    <article className="mx-auto max-w-2xl space-y-6">
      ...
      {user && (
        <div className="flex flex-wrap gap-2">
          ...
          {isOrganizer && (
            <>
              <a href={`/recruitments/${ev.id}/participants`} className="btn-outline">参加者管理</a>
              <a href={`/recruitments/${ev.id}/edit`} className="btn-outline">募集を修正</a>
            </>
          )}
          {isMember && isPast && (
            <a href={`/recruitments/${ev.id}/review`} className="btn-outline">相互評価する</a>
          )}
        </div>
      )}
      ...
      {isOrganizer ? (
        <div className="card p-4 text-sm text-slate-600">
          ...
          <a href={`/recruitments/${ev.id}/participants`} className="text-brand hover:underline">参加者管理</a>で運営できます。
        </div>
      ) : canCancel ? (
