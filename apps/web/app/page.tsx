    ...
    <div className="space-y-12">
      ...
      <section>
        <div className="mb-4 flex items-end justify-between">
          ...
          <Link href="/races" className="text-sm text-brand hover:underline">大会を探す →</Link>
        </div>
        {races.length === 0 ? (
            ...
            {races.map((r) => (
              <div key={r.id} className="card p-4">
                ...
                <div className="mt-2 flex gap-3 text-sm">
                  ...
                  <Link href={`/events/new?race=${encodeURIComponent(r.name)}${r.prefecture ? `&pref=${encodeURIComponent(r.prefecture)}` : ""}`} className="text-slate-500 hover:underline">仲間を募集</Link>
                </div>
              </div>
            ))}
          </div>
