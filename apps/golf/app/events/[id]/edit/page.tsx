...
/** ISO(UTC) を datetime-local 用の "YYYY-MM-DDTHH:mm" に変換（作成時と同じ素朴な丸め）。 */
...
export default async function EditEventPage({
  ...
  const { data: ev } = await supabase
    ...
    .from("events")
    .select(
      "id, organizer_id, title, description, prefecture, city, event_start_at, application_deadline, capacity, participation_fee, beginner_allowed, approval_type, gender_condition, skill_level, condition_prefectures, condition_sport_ids, deleted_at",
    )
    .eq("id", id)
    .maybeSingle();
  ...
  const event: EventInit = {
    ...
    application_deadline_local: ev.application_deadline ? toLocalInput(ev.application_deadline) : "",
