...
export default async function FollowersPage() {
  ...
  if (!user) redirect("/login?redirect=/mypage/followers");

  const users = await fetchFollows(supabase, user.id, "followers");
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      {users.length === 0 ? (
        <p className="text-sm text-slate-400">フォロワーはいません。</p>
      ) : (
