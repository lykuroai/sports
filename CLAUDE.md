...
- **【単一オリジン】サブドメイン跨ぎ（Cookie 共有・絶対URLログイン誘導）は廃止**。ログイン・課金・
  公開プロフィール・通知設定・本人確認・退会は web 自身の相対パス（`/login`・`/owner/login` 等）で完結。
  `NEXT_PUBLIC_ACCOUNT_URL`/`FACILITY_URL`/`COOKIE_DOMAIN` は**未設定（空）が前提**。auth-client は空なら
  `loginUrl()`→相対 `/login`、proxy も自オリジン `/login` に倒れる。OAuth/メール確認/LINE/Stripe の
  コールバックは絶対URLが要るため `selfOrigin()`/`requestOrigin()`（X-Forwarded-Host 由来）で組み立てる。
...
- **ログイン後の戻り先（redirect）**: 未ログインで保護ページ（募集作成・参加申請・編集等）へ
  ...
  - 保護パスの未ログイン誘導は web 内の相対 `/login`（運営者領域 `/owner`・`/facilities/submit` は
...
- 施設運営者は **web 内の運営者専用ルートで登録/ログイン**（`apps/web/app/owner/register|login`、
  種別を `signUp` の `options.data.account_type='facility_owner'` で付与。`handle_new_user`
  トリガーが `account.users.account_type` を設定）。一般の `/login`・`/register` とは名前空間分離。
- 施設運営者は **一般プロフィール `account.profiles` を持たない**（トリガーが種別で出し分け）。
  → 一般機能（プロフィール・募集参加）は使えない。`requireGeneralAccount()` で弾く。
- 施設運営者専用領域（`/owner`・`/facilities/submit`）は `requireOwnerAccount()` でガード
  （`account_type='facility_owner'` または既存 verified オーナー）。未ログイン誘導は
  `apps/web/proxy.ts` がパス分岐し `loginPath:"/owner/login"` を使う。
- 既存ユーザは非破壊で `general` のまま（ロックアウト防止）。既存オーナーは当面 dual-use。
- 単一オリジン運用のため Cookie は host-only（`COOKIE_DOMAIN` 未設定）。種別で機能分離する。
  FK（`facility_owners.user_id → account.users`）は維持。
- 未対応: 運営者の Google/LINE/電話 登録（その経路は `general` になる）。
